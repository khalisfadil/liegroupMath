...
// boolean test value
bool covAccessDidRaise(math::se3::TransformationWithCovariance& T) {
  bool raised = false;
...
Eigen::Matrix<double, 6, 6> covSafe(
    math::se3::TransformationWithCovariance& T) {
...
#define CHECK_EQ_COVARIANCE(T, U)       \
  std::cout << "true cov: \n"           \
            << U << "\ntest cov: \n"    \
            << covSafe(T) << std::endl; \
  EXPECT_TRUE(math::common::nearEqual(U, covSafe(T), 1e-6));

// Checks that two matrices are equal
#define CHECK_EQ(A, B)                                                    \
  std::cout << "true mat: \n" << A << "\ntest mat: \n" << B << std::endl; \
  EXPECT_TRUE(math::common::nearEqual(A, B, 1e-6));
...
TEST(LGMath, TransformationWithCovarianceConstructors) {
  ...
  math::se3::Transformation randBase(C_ba, r_ba_ina);
  math::se3::TransformationWithCovariance rand(C_ba, r_ba_ina, U);

  // TransformationWithCovariance();
  // default"
  {
    math::se3::TransformationWithCovariance tmatrix;
  ...
  // copy constructor"
  {
    math::se3::TransformationWithCovariance test(rand);
  ...
  {
    math::se3::TransformationWithCovariance test(randBase, U);
  ...
  // matrix constructor"
  {
    math::se3::TransformationWithCovariance test(rand.matrix());
    ...
    math::se3::TransformationWithCovariance test_bad(
  ...
  // matrix constructor with covariance"
  {
    ...
    notTransform.topLeftCorner<3, 3>() = notRotation;
    notTransform.topRightCorner<3, 1>() = -r_ba_ina;
    math::se3::TransformationWithCovariance test_bad(notTransform,
                                                       U);  // force reproj
  ...
  // assignment operator"
  {
    math::se3::TransformationWithCovariance test;
    test = rand;
    ...
    EXPECT_TRUE(typeid(test) ==
                typeid(math::se3::TransformationWithCovariance));
  }
  ...
  {
    math::se3::TransformationWithCovariance test;
    test = math::se3::TransformationWithCovariance();
  ...
  // assignment operator of base transform to subclass"
  {
    ...
    CHECK_EQ(rand.matrix(), test.matrix());
    EXPECT_TRUE(typeid(test) == typeid(math::se3::Transformation));
  }
  ...
  // int numTerms = 0); exponential map"
  {
    ...
    math::se3::TransformationWithCovariance testAnalytical(vec);
    math::se3::TransformationWithCovariance testNumerical(vec, 15);
  ...
  // covariance"
  {
    Eigen::Matrix<double, 6, 1> vec = Eigen::Matrix<double, 6, 1>::Random();
    Eigen::Matrix4d tmat = math::se3::vec2tran(vec);
    math::se3::TransformationWithCovariance testAnalytical(vec, U);
    math::se3::TransformationWithCovariance testNumerical(vec, U, 15);
  ...
  // exponential map with VectorXd"
  {
    ...
    Eigen::Matrix4d tmat = math::se3::vec2tran(vec);
    math::se3::TransformationWithCovariance test(vec);
  ...
  // U); exponential map with VectorXd, with covariance"
  {
    Eigen::VectorXd vec = Eigen::Matrix<double, 6, 1>::Random();
    Eigen::Matrix4d tmat = math::se3::vec2tran(vec);
    math::se3::TransformationWithCovariance test(vec, U);
  ...
  // exponential map with bad VectorXd"
  {
    ...
    math::se3::TransformationWithCovariance testFailure;
    try {
      testFailure = math::se3::TransformationWithCovariance(badvec, U);
  ...
  {
    math::se3::TransformationWithCovariance test(C_ba, r_ba_ina);
  ...
  {
    math::se3::TransformationWithCovariance test(C_ba, r_ba_ina, U);
    Eigen::Matrix4d tmat = Eigen::Matrix4d::Identity();
    tmat.topLeftCorner<3, 3>() = C_ba;
    tmat.topRightCorner<3, 1>() = -C_ba * r_ba_ina;
  ...
  // move constructor"
  {
    math::se3::TransformationWithCovariance test(std::move(rand));
  ...
  // move constructor (subclass from base)"
  {
    ...
    math::se3::TransformationWithCovariance test(std::move(randBase));
    randBase = randBase2;
...
/////////////////////////////////////////////////////////////////////////////////////////////
TEST(math, TransformationWithCovarianceGetMethods) {
  // Generate random transform from most basic constructor
  Eigen::Matrix<double, 3, 3> C_ba =
      math::so3::vec2rot(Eigen::Matrix<double, 3, 1>::Random());
  Eigen::Matrix<double, 3, 1> r_ba_ina = Eigen::Matrix<double, 3, 1>::Random();
  Eigen::Matrix<double, 6, 6> U = Eigen::Matrix<double, 6, 6>::Random();
  math::se3::TransformationWithCovariance T_ba(C_ba, r_ba_ina, U);
...
/////////////////////////////////////////////////////////////////////////////////////////////
TEST(math, TransformationWithCovarianceOperations) {
  // Generate random transform from most basic constructor
  ...
  math::se3::TransformationWithCovariance T1(C1, r1, U1);
  ...
  math::se3::TransformationWithCovariance T2(C2, r2, U2);

  Eigen::Matrix<double, 3, 3> C3 =
      math::so3::vec2rot(Eigen::Matrix<double, 3, 1>::Random());
  Eigen::Matrix<double, 3, 1> r3 = Eigen::Matrix<double, 3, 1>::Random();
  math::se3::TransformationWithCovariance T3(C3, r3);

  Eigen::Matrix<double, 3, 3> C4 =
      math::so3::vec2rot(Eigen::Matrix<double, 3, 1>::Random());
  Eigen::Matrix<double, 3, 1> r4 = Eigen::Matrix<double, 3, 1>::Random();
  math::se3::Transformation T4(C4, r4);

  /// Operator: *=

  // test TWC *= TWC
  {
    math::se3::TransformationWithCovariance test(T1);
    test *= T2;
  ...
  // test T *= TWC
  {
    math::se3::Transformation test(T4);
    test *= T1;
    ...
    EXPECT_TRUE(
        typeid(test) ==
        typeid(
            math::se3::Transformation));  // Make sure nothing weird happened
  }
  ...
  // test TWC(unset) *= TWC
  {
    math::se3::TransformationWithCovariance test(T3);
    test *= T1;
  ...
  // test TWC * TWC
  {
    math::se3::TransformationWithCovariance test = T1 * T2;
    Eigen::Matrix4d tmat = T1.matrix() * T2.matrix();
  ...
  // test TWC * T
  {
    math::se3::TransformationWithCovariance test = T1 * T4;
    Eigen::Matrix4d tmat = T1.matrix() * T4.matrix();
  ...
  // test T * TWC
  {
    math::se3::TransformationWithCovariance test = T4 * T1;
    Eigen::Matrix4d tmat = T4.matrix() * T1.matrix();
  ...
  // test TWC * TWC(unset)
  {
    math::se3::TransformationWithCovariance test = T1 * T3;
    Eigen::Matrix4d tmat = T1.matrix() * T3.matrix();
  ...
  // test TWC(unset) * TWC
  {
    math::se3::TransformationWithCovariance test = T3 * T1;
    Eigen::Matrix4d tmat = T3.matrix() * T1.matrix();
  ...
  // test TWC(unset) * TWC(unset)
  {
    math::se3::TransformationWithCovariance test = T3 * T3;
    Eigen::Matrix4d tmat = T3.matrix() * T3.matrix();
  ...
  // test TWC(unset) * T
  {
    math::se3::TransformationWithCovariance test = T3 * T4;
    Eigen::Matrix4d tmat = T3.matrix() * T4.matrix();
  ...
  // test T * TWC(unset)
  {
    math::se3::TransformationWithCovariance test = T4 * T3;
    Eigen::Matrix4d tmat = T4.matrix() * T3.matrix();
  ...
  // test TWC inverse
  {
    math::se3::TransformationWithCovariance test = T1.inverse();
  ...
  {
    math::se3::TransformationWithCovariance T5(C3, r3);
    T5.setCovariance(U2);
